...
static void Cmd_getexp(void)
{
    ...
    case 1: // calculate experience points to redistribute
        {
            ...
            {
                if (GetMonData(&gPlayerParty[i], MON_DATA_SPECIES) == SPECIES_NONE || GetMonData(&gPlayerParty[i], MON_DATA_IS_EGG) || GetMonData(&gPlayerParty[i], MON_DATA_HP) == 0 || GetMonData(&gPlayerParty[i], MON_DATA_LEVEL) == MAX_LEVEL || ((gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER) && i >= 3))
                    continue;
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (gExpTeamCheck == ((gBattleStruct->sentInPokes & (1 << gBattleStruct->expGetterMonId)) != 0))
            {
    ...
    case 5: // looper increment
        ...
        else
        {
            ...
            else
            {
                ...
                for (viaSentIn = 0, i = 0; i < PARTY_SIZE; i++)
                {
                    if (GetMonData(&gPlayerParty[i], MON_DATA_SPECIES) == SPECIES_NONE || GetMonData(&gPlayerParty[i], MON_DATA_IS_EGG) || GetMonData(&gPlayerParty[i], MON_DATA_HP) == 0)
                        continue;
                    totalMon++;
                    if (gBitTable[i] & sentIn)
                        viaSentIn++;
                ...
                if (!gExpTeamCheck && totalMon > viaSentIn)
                {
                    gExpTeamCheck = TRUE;
                    gBattleStruct->expGetterMonId = 0;
